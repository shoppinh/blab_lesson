...
KeyPair.prototype.onExecute = function () {
  ...
  if (
    ...
    optionalPrivateKey != this.privateKey
  ) {
    if (
      optionalPrivateKey &&
      typeof optionalPrivateKey.indexOf == "function" &&
      optionalPrivateKey.indexOf("0x") < 0
    ) {
...
KeyPair.prototype.updateAddress = async function () {
  try {
    if (this.privateKey) {
      ...
      this.publicKey =
        "0x" + EthUtil.privateToPublic(this.privateKey).toString("hex");
      this.address =
        "0x" + EthUtil.privateToAddress(this.privateKey).toString("hex");
    } else {
