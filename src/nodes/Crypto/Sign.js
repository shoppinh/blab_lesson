...
Sign.prototype.onExecute = async function () {
  ...
  if (
    typeof optionalMessage != "undefined" &&
    optionalMessage != this.properties.message
  ) {
    this.onPropertyChanged("message", optionalMessage);
  }
  this.setOutputData(
    0,
    this.address ? this.address.toLowerCase() : this.address
  );
...
Sign.prototype.sign = async function () {
  ...
  if (this.signatures[this.address + this.properties["message"]]) {
    ...
    try {
      this.signature = await this.web3.eth.sign(
        this.properties["message"],
        this.address
      );
