...
Transaction.prototype.onExecute = function () {
  ...
  if (
    typeof optionalProvider != "undefined" &&
    optionalProvider != this.properties.provider
  ) {
  ...
  if (
    typeof optionalPrivateKey != "undefined" &&
    optionalPrivateKey != this.properties.privateKey
  ) {
  ...
  if (
    ...
    optionalValue != this.properties.value
  ) {
    this.onPropertyChanged("value", optionalValue);
  }
  let optionalData = this.getInputData(4);
  if (
    typeof optionalData != "undefined" &&
    optionalData != this.properties.data
  ) {
    if (
      optionalData &&
      typeof optionalData.indexOf == "function" &&
      optionalData.indexOf("0x") < 0
    ) {
  ...
  if (
    typeof optionalGas != "undefined" &&
    optionalGas != this.properties.data
  ) {
    this.onPropertyChanged("gas", optionalGas);
  }
  let optionalGasPrice = this.getInputData(6);
  if (
    typeof optionalGasPrice != "undefined" &&
    optionalGasPrice != this.properties.data
  ) {
  ...
  try {
    ...
    if (
      (nonce == null ||
        ...
        typeof this.getInputData(7) == "undefined") &&
      ...
      try {
        let publicAddress =
          "0x" +
          EthUtil.privateToAddress(this.properties.privateKey).toString("hex");
