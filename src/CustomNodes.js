  ...
  obj.prototype.onOutputDblClick = function (index, e) {
    ...
    if (this.outputs[index].type == -1) {
      ...
      if (
        ...
        var node_watch =
          globalLiteGraphJS.LiteGraph.createNode("Display/Address");
        node_watch.pos = [e.canvasX + 90, e.canvasY - 25];
        this.graph.add(node_watch);
        this.connect(index, node_watch, 0);
      } else if (
        ...
        var node_watch =
          globalLiteGraphJS.LiteGraph.createNode("Display/TextArea");
        node_watch.pos = [e.canvasX + 90, e.canvasY - 25];
        this.graph.add(node_watch);
        this.connect(index, node_watch, 0);
      } else if (
        ...
        var node_object =
          globalLiteGraphJS.LiteGraph.createNode("Object/Object");
        ...
        var node_watch =
          globalLiteGraphJS.LiteGraph.createNode("Display/Watch");
        node_watch.pos = [e.canvasX + 90, e.canvasY - 25];
        this.graph.add(node_watch);
        this.connect(index, node_watch, 0);
        ...
        try {
          ...
          if (value > 0.0001 && value < 1000000000) {
            ...
            var node_from_wei =
              globalLiteGraphJS.LiteGraph.createNode("Utils/From Wei");
            ...
            if (value > 0.01 && value < 9999) {
              //do a conversion to wei
              var node_from_wei =
                globalLiteGraphJS.LiteGraph.createNode("Utils/From Gwei");
              node_from_wei.pos = [e.canvasX + 40, e.canvasY];
              this.graph.add(node_from_wei);
              this.connect(index, node_from_wei, 0);
              node_from_wei.connect(0, node_watch, 0);
  ...
  obj.prototype.onInputDblClick = function (index, e) {
    ...
    if (this.inputs[index].type == -1) {
      if (this.type == "Input/Button") {
        ...
        var node_button =
          globalLiteGraphJS.LiteGraph.createNode("Control/Timer");
        ...
        var node_button =
          globalLiteGraphJS.LiteGraph.createNode("Input/Button");
    ...
    {
      console.log(
        "unable to find automatic pair for ",
        this.inputs[index].type
      );
    }
  };
...
global.customNodes = [
  { name: "Input", color: "b9a814", icon: "✏️", displayName: "Input" },
  { name: "Display", color: "b9a814", icon: "🖥", displayName: "Display" },
  { name: "Storage", color: "b9a814", icon: "💾", displayName: "Storage" },
  // { name: "Network", color: "b9a814", icon: "📡", displayName: "Network" },
  { name: "Crypto", color: "b9a814", icon: "🔐", displayName: "Crypto" },
  { name: "Web3", color: "b9a814", icon: "🦄", displayName: "Web3" },
