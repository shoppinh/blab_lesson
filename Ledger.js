  ...
  try {
    if (tx.to && tx.from && tx.value) {
      if (!genesis || tx.from != "0x0000000000000000000000000000000000000000") {
        this.balances[tx.from] = this.balances[tx.from]
          ? this.balances[tx.from] - tx.value
          : -tx.value;
      }
      this.balances[tx.to] = this.balances[tx.to]
        ? this.balances[tx.to] + tx.value
        : tx.value;
      this.txns.push(tx);
    }
  } catch (e) {
      ...
      rows.push(
        <StyledTableRow>
          <TableCell style={rowStyle}>
            <Blockies seed={tx.from.toLowerCase()} size={8} scale={2} />
          ...
          <TableCell style={rowStyle}>
            <Blockies seed={tx.to.toLowerCase()} size={8} scale={2} />
